...
const productModalblock = document.querySelector('.productModal__block')

closeModal.addEventListener("click", () => {
    modal.classList.remove("show")
    productModalblock.classList.remove("show")
})
...
function renderProducts(products) {
    ...
    products.forEach(product => {
        ...
        card.innerHTML = `
            ...
            <div class="product-info">
                ...
                <div class="product-box">
                    ...
                    <span class="material-symbols-outlined" style="color: green" onclick="editProduct(${product.id}, event)">
                        edit
                    </span>
                    <span class="material-symbols-outlined" style="color: red" onclick="deleteProduct(${product.id}, event)">
                        delete
                    </span>
                </div>
            </div>
        ...
        card.addEventListener("click", (event) => clickEvent(event, product))
        container.appendChild(card)
    })
}

function clickEvent(event, product) {
    if (event.target.closest('.fa') || event.target.closest('.material-symbols-outlined')) {
        return
    }
    showModal(product)
}
...
function showModal(product) {
    modalBody.innerHTML = `
        ...
        <div class="modal-product-info">
            ...
            <button class="modal__btn" onclick='saveProduct(${product.id})'>Save Product</button>
        </div>
    `
...
function editProduct(productId, event) {
    ...
    productModalblock.innerHTML = `
        <div class="productModal">
            ...
            <input type="number" id="discount" value="${product.discountPercentage}">
            <label for="in-stock">In stock:</label>
            <input type="number" id="in-stock" value="${product.stock}">
            <label for="brand">Brand:</label>
            <input type="text" id="brand" value="${product.brand}">
            <label for="category">Category:</label>
            <input type="text" id="category" value="${product.category}">
            <label for="image">Image:</label>
            <input type="url" id="image" value="${product.thumbnail}">
            <button class="productModal-btn" onclick="saveEditedProduct(${product.id})">Save Product</button>
        </div>
    `
    productModalblock.classList.add("show")
}
...
function saveEditedProduct(productId) {
    ...
    if (productIndex !== -1) {
        const updatedProduct = {
            ...
            stock: parseInt(document.getElementById('in-stock').value, 10),
...
function deleteProduct(productId, event) {
    ...
    if (confirm("Do you really want to remove " + product.title + " element?")) {
        const updatedProducts = allProducts.filter(prod => prod.id !== productId)
